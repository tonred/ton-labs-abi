...
use ton_types::{fail, BuilderData, Cell, HashmapE, IBitstring, Result};
...
impl TokenValue {
    ...
    fn write_optional(param_type: &ParamType, value: Option<&TokenValue>, abi_version: u8) -> Result<Vec<BuilderData>> {
        if let Some(value) = value {
            ...
            Ok(vec![BuilderData::with_raw(smallvec![0x00], 1)?])
        }
...
fn test_pack_cells() {
    let cells = vec![
        BuilderData::with_bitstring(smallvec![1, 2, 0x80]).unwrap(),
        BuilderData::with_bitstring(smallvec![3, 4, 0x80]).unwrap(),
    ];
    ...
    let cells = vec![
        BuilderData::with_raw(smallvec![0x55; 100], 100 * 8).unwrap(),
        BuilderData::with_raw(smallvec![0x55; 127], 127 * 8).unwrap(),
        BuilderData::with_raw(smallvec![0x55; 127], 127 * 8).unwrap(),
    ];

    let builder = BuilderData::with_raw(smallvec![0x55; 127], 127 * 8).unwrap();
    let builder =
        BuilderData::with_raw_and_refs(smallvec![0x55; 127], 127 * 8, [builder.into()]).unwrap();
    let builder =
        BuilderData::with_raw_and_refs(smallvec![0x55; 100], 100 * 8, [builder.into()]).unwrap();
